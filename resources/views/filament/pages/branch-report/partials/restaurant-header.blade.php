...
<div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 overflow-hidden">
    ...
    <div class="relative h-40 sm:h-52 rounded-t-2xl" style="background: linear-gradient(to bottom right, rgb(59 130 246), rgb(79 70 229));">
        <div class="absolute inset-0 rounded-t-2xl" style="background: rgba(0,0,0,0.15);"></div>
        <div class="absolute inset-0 p-6 sm:p-8 flex items-end">
            <div class="flex items-end gap-5 sm:gap-6 w-full">
                {{-- Restaurant Image --}}
                <div class="w-20 h-20 sm:w-28 sm:h-28 rounded-2xl overflow-hidden border-4 border-white shadow-xl bg-white flex-shrink-0">
                    ...
                    @else
                        <div class="w-full h-full flex items-center justify-center" style="background: rgb(239 246 255);">
                            <x-heroicon-o-building-storefront class="w-10 h-10 sm:w-14 sm:h-14 text-blue-600" />
                        </div>
                    @endif
                </div>

                {{-- Restaurant Info --}}
                <div class="flex-1 text-white min-w-0 pb-1">
                    <h1 class="text-2xl sm:text-3xl font-bold mb-2 sm:mb-3 truncate">
                        {{ $restaurantInfo['name'] }}
                    </h1>
