@php
    $data = $card['data'] ?? [];
    $categories = $data['dynamicCategories'] ?? $data['organicCategories'] ?? $data['categories'] ?? [];
    $categoriesArray = is_array($categories) ? array_values(array_filter($categories, fn($c) => isset($c['name']))) : [];

    // Find best and worst
    $bestCategory = !empty($categoriesArray) ? collect($categoriesArray)->sortByDesc('rating')->first() : null;
    $worstCategory = !empty($categoriesArray) ? collect($categoriesArray)->sortBy('rating')->first() : null;
@endphp
...
<div class="space-y-6">
    ...
    @foreach($categoriesArray as $index => $category)
        @php
            ...
            $positiveQuotes = $category['topPhrasesPositive'] ?? [];
            $negativeQuotes = $category['topPhrasesNegative'] ?? [];
            ...
            <div class="space-y-3">
                @if(count($positiveQuotes) > 0)
                    <div class="bg-green-50 dark:bg-green-900/20 border-r-4 border-green-400 dark:border-green-600 p-3 rounded">
                        ...
                        <p class="text-sm text-green-800 dark:text-green-200">"{{ $positiveQuotes[0] }}"</p>
                    </div>
                @endif

                @if(count($negativeQuotes) > 0)
                    <div class="bg-red-50 dark:bg-red-900/20 border-r-4 border-red-400 dark:border-red-600 p-3 rounded">
                        ...
                        <p class="text-sm text-red-800 dark:text-red-200">"{{ $negativeQuotes[0] }}"</p>
                    </div>
                @endif
            </div>
        </div>
    ...
    @if($bestCategory && $worstCategory && $bestCategory !== $worstCategory)
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div class="bg-gradient-to-br from-green-50 to-green-100/50 dark:from-green-900/20 dark:to-green-800/20 rounded-xl p-4 border border-green-200 dark:border-green-800">
                ...
                <div class="text-lg font-bold text-gray-900 dark:text-white">{{ $bestCategory['name'] }}</div>
                <div class="text-2xl font-bold text-green-600 dark:text-green-400">{{ number_format($bestCategory['rating'] ?? 0, 1) }} <span class="text-amber-400">★</span></div>
            </div>
            ...
            <div class="bg-gradient-to-br from-red-50 to-red-100/50 dark:from-red-900/20 dark:to-red-800/20 rounded-xl p-4 border border-red-200 dark:border-red-800">
                ...
                <div class="text-lg font-bold text-gray-900 dark:text-white">{{ $worstCategory['name'] }}</div>
                <div class="text-2xl font-bold text-red-600 dark:text-red-400">{{ number_format($worstCategory['rating'] ?? 0, 1) }} <span class="text-amber-400">★</span></div>
            </div>
        </div>
    @endif
</div>
